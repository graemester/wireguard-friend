# V2 Vision - Unified Semantic Model

**Core insight:** Semantic understanding is just well-named columns.

No "layers" - just tables with good attributes that get populated during import.
...
**Answer:** Store them as **command pairs** with semantic attributes.

```sql
command_pair (
    pattern_name TEXT,    -- 'nat_masquerade_ipv4'
    rationale TEXT,       -- 'NAT for VPN subnet (IPv4)'
    scope TEXT,           -- 'environment-wide'
    up_commands TEXT,     -- JSON: ["iptables -A ...", ...]
    down_commands TEXT,   -- JSON: ["iptables -D ...", ...]
    variables TEXT        -- JSON: {"wan_iface": "eth0"}
)
```

**Provenance:**
- Commands are **paired** (1:1 up/down relationship, or singleton if no down)
- Pairs share a **rationale** (why they exist)
- Pairs have **scope** (environment-wide or peer-specific)
- Pattern recognizer runs during import, populates these attributes

**Implementation:** `v2/patterns.py` - 100% recognition on real configs ✓

---

### 2. How do you store "comment attached to Peer 3, line 2"?

**Answer:** Comments are **entity attributes** with semantic categories.

```sql
comment (
    entity_type TEXT,     -- 'coordination_server', 'subnet_router', 'remote'
    entity_id INT,        -- Links to specific entity
    category TEXT,        -- 'hostname', 'role', 'custom'
    text TEXT,
    role_type TEXT,       -- 'initiates_only', 'dynamic_endpoint'
    display_order INT     -- hostname=1, role=2, custom=999
)
```

**Categories:**
- **hostname** - Enforced human-readable unique ID (BEFORE entity)
- **role** - Function/characteristics (BEFORE entity)
  - `initiates_only` - "no endpoint == behind CGNAT"
  - `dynamic_endpoint` - "mobile device"
- **rationale** - Why commands exist (BEFORE command groups)
- **custom** - Personal admin notes (AFTER entity)
  - "I rotate this key on Sundays..."
  - "I added preshared key when brother spoofed..."

**Order:** hostname → role → entity → custom

**Implementation:** `v2/comments.py` - Auto-categorization working ✓

---

### 3. How do you capture "user prefers 2 blank lines between peers"?

**Answer:** You don't. Enforce conventions.

```python
# wireguard-friend has ONE canonical style
STANDARD_FORMAT = {
    "blank_lines_between_peers": 1,
    "indent": "spaces",
    "indent_width": 4,
    "field_order": CANONICAL_ORDER
}
```

**No format detection. No user preferences. Just standards.**

---

### 4. What about unknown future WireGuard fields?

**Answer:** System is inherently extensible - just add a column.

```sql
-- WireGuard adds "RoutingTable" field?
ALTER TABLE coordination_server ADD COLUMN routing_table TEXT;
ALTER TABLE subnet_router ADD COLUMN routing_table TEXT;
ALTER TABLE remote ADD COLUMN routing_table TEXT;

-- Update parser (one line):
elif field_lower == 'routingtable':
    entity.routing_table = field_value
```

**No special unknown field handling.** When WireGuard evolves, we evolve.
